package com.allen.imsystem.user.controller;
...
import com.allen.imsystem.user.model.param.RegistFormParam;
import com.allen.imsystem.user.model.pojo.User;
import com.allen.imsystem.user.service.SecurityService;
...
//        securityService.verifyImageCode(code,correctCode);
    ...
    public JSONResponse regist(@RequestBody @Validated RegistFormParam registFormParam) {
        String email = registFormParam.getEmail();
        String password = registFormParam.getPassword();
        String username = registFormParam.getUsername();
        String code = registFormParam.getCode();
        String emailCode = registFormParam.getEmailCode();
...
//        securityService.verifyImageCode(code,correctCode);
    ...
    public JSONResponse updateUserInfo(@RequestBody EditUserInfoParam editUserInfoParam, HttpServletRequest request) {
        String uid = cacheHolder.getUid(request);
        editUserInfoParam.setUid(uid);
        userService.updateUserInfo(editUserInfoParam, uid);
        return new JSONResponse(1);
    }
    ...
    public JSONResponse getSelfInfo(HttpServletRequest request) {
        ...
        EditUserInfoParam selfInfo = userService.getSelfInfo(userId);
