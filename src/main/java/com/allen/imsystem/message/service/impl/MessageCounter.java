...
public class MessageCounter {
    ...
    public boolean setUserChatNewMsgCount(String uid, Long chatId, Integer count) {
        ...
        return redisService.hset(GlobalConst.RedisKey.KEY_CHAT_UNREAD_COUNT, uid + chatId, count);
    }
    ...
    public Integer getPrivateChatNewMsgCount(String uid, Long chatId) {
        Integer count = (Integer) redisService.hget(GlobalConst.RedisKey.KEY_CHAT_UNREAD_COUNT, uid + chatId);
    ...
    public Integer getUserGroupChatNewMsgCount(String uid, String gid) {
        Integer count = (Integer) redisService.hget(GlobalConst.RedisKey.KEY_CHAT_UNREAD_COUNT, uid + gid);
    ...
    public void incrPrivateChatNewMsgCount(String uid, Long chatId) {
        ...
        hasKey = redisService.hHasKey(GlobalConst.RedisKey.KEY_CHAT_UNREAD_COUNT, uid + chatId);
        if (!hasKey) {
            // 去数据库里同步。
            Integer newMsgCount = privateMsgRecordMapper.countPrivateChatUnReadMsgForUser(chatId, uid);
        ...
        Long val = redisService.hincr(GlobalConst.RedisKey.KEY_CHAT_UNREAD_COUNT, uid + chatId, 1L);
        return;
    }
